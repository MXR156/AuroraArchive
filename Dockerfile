...
RUN case "$TARGETARCH" in amd64) YTDLP_ARCH=""; DENO_ARCH="x86_64" ;; arm64) YTDLP_ARCH="_aarch64"; DENO_ARCH="aarch64" ;; *) exit 1 ;; esac \
    && case "$YT_DLP_CHANNEL" in stable) YTDLP_REPOSITORY="yt-dlp/yt-dlp" ;; nightly) YTDLP_REPOSITORY="yt-dlp/yt-dlp-nightly-builds" ;; *) echo "Unsupported yt-dlp channel: $YT_DLP_CHANNEL" >&2; exit 1 ;; esac \
    && curl -fsSL "https://github.com/${YTDLP_REPOSITORY}/releases/latest/download/yt-dlp_linux${YTDLP_ARCH}" -o /usr/local/bin/yt-dlp \
...
ENV APP_NAME=AuroraArchive \
    ...
    TEMP=/var/www/html/storage/app/tmp \
    PYTHONHASHSEED=0 \
    TZ=Europe/London \
    PUID=1000 \
    PGID=1000 \
